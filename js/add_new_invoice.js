...
class MedicineInfo {
  constructor(name, batch_id, expiry_date, quantity, mrp, discount, total, product_id) {
    this.name = name;
...
function addInvoice() {
  ...
  if(!notNull(customers_name.value, "customer_name_error"))
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      var product_id = elements[1].children[0].children[0];
      ...
      if(!notNull(medicine_name.value, medicine_name_error.getAttribute('id')))
        ...
        flag = true;
        //alert("row " + i + "perfect...");
        medicines[i-1] = new MedicineInfo(medicine_name.value, batch_id.value, expiry_date.value, quantity.value, mrp.value, discount.value, total.value, product_id.value);
      }
    ...
    for(var i = 0; i < row_count - 1; i++) {
      ...
      addSale(customers_name.value, customers_contact_number.value, invoice_number.value, medicines[i].quantity, medicines[i].discount, medicines[i].total, medicines[i].product_id);
      //alert(invoice_number.value);
    }
    addNewInvoice(invoice_number.value, customers_name.value, customers_contact_number.value, invoice_date.value, total_amount.value, total_discount.value, net_total.value, product_id.value);
    //alert(invoice_number.value);
...
function addSale(customers_name, customers_contact_number, invoice_number, quantity, discount, total, product_id) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      xhttp.responseText;
  ...
  xhttp.open("GET", "php/add_new_invoice.php?action=add_sale&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_number=" + invoice_number +  "&quantity=" + quantity + "&discount=" + discount + "&total=" + total + "&product_id=" + product_id, true);
  xhttp.send();
}

function addNewInvoice(invoice_number, customers_name, customers_contact_number, invoice_date, total_amount, total_discount, net_total, product_id) {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if(xhttp.readyState = 4 && xhttp.status == 200)
      document.getElementById("invoice_acknowledgement").innerHTML =  xhttp.responseText;
  };
  xhttp.open("GET", "php/add_new_invoice.php?action=add_new_invoice&invoice_number=" + invoice_number + "&customers_name=" + customers_name + "&customers_contact_number=" + customers_contact_number + "&invoice_date=" + invoice_date + "&total_amount=" + total_amount + "&total_discount=" + total_discount + "&net_total=" + net_total + "&product_id=" + product_id, true);
  xhttp.send();
}
