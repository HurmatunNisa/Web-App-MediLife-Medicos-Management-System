<?php
  ...
  function showPurchaseRow($seq_no, $row) {
    ...
    <tr>
      ...
      -->
        <button href="" class="btn btn-info btn-sm" onclick="editPurchase(<?php echo $row['INVOICE_NUMBER']; ?>);">
...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    ...
    <td>
      <button href="" class="btn btn-success btn-sm" onclick="updatePurchase(<?php echo $row['INVOICE_NUMBER']; ?>);">
...
function updatePurchase($id, $suppliers_name, $invoice_date, $grand_total, $payment_status) {
  require "db_connection.php";
  //echo $payment_status;
  $query = "UPDATE purchases SET SUPPLIER_NAME = '$suppliers_name', PURCHASE_DATE = '$invoice_date', TOTAL_AMOUNT = $grand_total, PAYMENT_STATUS = '$payment_status' WHERE INVOICE_NUMBER = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
    showPurchases(0);
}
