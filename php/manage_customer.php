<?php
  ...
  if($con) {
    ...
    if(isset($_GET["action"]) && $_GET["action"] == "update") {
      ...
      updateCustomer($id, $name, $contact_number, $address, $email, $gender, $doctor_name, $doctor_address);
    }
...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    ...
    <td>
      <input type="email" class="form-control" value="<?php echo $row['EMAIL']; ?>" placeholder="Email" id="customer_email" onkeyup="validateEmail(this.value, 'email_error');">
      <code class="text-danger small font-weight-bold float-right" id="email_error" style="display: none;"></code>
    </td>
    <td>
      <input type="text" class="form-control" value="<?php echo $row['GENDER']; ?>" placeholder="Gender" id="customer_gender">
...
function updateCustomer($id, $name, $contact_number, $address, $email, $gender, $doctor_name, $doctor_address) {
  require "db_connection.php";
  $query = "UPDATE customers SET NAME = '$name', CONTACT_NUMBER = '$contact_number', ADDRESS = '$address', EMAIL = '$email', GENDER = '$gender', DOCTOR_NAME = '$doctor_name', DOCTOR_ADDRESS = '$doctor_address' WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
    showCustomers(0);
}
