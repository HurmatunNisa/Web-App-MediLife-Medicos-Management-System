...
function isAdmin() {
    ...
    if ($con) {
        ...
        $query = "SELECT TYPE FROM admin_credentials WHERE USERNAME = '$username' AND PASSWORD = '$password'";
        $result = mysqli_query($con, $query);
        ...
        if ($result && mysqli_num_rows($result) === 1) {
            ...
            // Start a session to store the user's role
            session_start();
            $_SESSION['TYPE'] = $role;
  ...
  if(isset($_GET['action']) && $_GET['action'] == 'update_username_password')
    ...
    function updateUsernamePassword() {
      ...
      if ($con) {
        ...
        // Check if the username exists in admin_credentials table
        $check_query = "SELECT * FROM admin_credentials WHERE USERNAME = '$username'";
        $check_result = mysqli_query($con, $check_query);
    
        if (mysqli_num_rows($check_result) > 0) {
          // Username exists, update the password in admin_credentials
          $update_admin_query = "UPDATE admin_credentials SET PASSWORD = '$password' WHERE USERNAME = '$username'";
          $update_admin_result = mysqli_query($con, $update_admin_query);
    
          // Update the password in the user table
          $update_user_query = "UPDATE user SET PASSWORD = '$password' WHERE USERNAME = '$username'";
          $update_user_result = mysqli_query($con, $update_user_query);
    
          if ($update_admin_result && $update_user_result) {
            echo "true"; // Password updated successfully in both tables
          } else {
            echo "false"; // Password update failed
          }
        } else {
          echo "username_not_found"; // Username not found in admin_credentials
        }
      }
    }
