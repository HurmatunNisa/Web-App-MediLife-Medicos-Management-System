<?php
  ...
  function createMedicineInfoRow() {
    ...
    <div class="row col col-md-8">
          <div class="col col-md-3">
            <input type="number" class="form-control" id="medicines_id_<?php echo $row_number; ?>" disabled hidden>
          </div>
      </div>
  ...
  function getInvoiceNumber() {
    ...
    require 'db_connection.php';
    if ($con) {
        $query = "SELECT MAX(INVOICE_ID) AS last_invoice_number FROM invoices;";
        $result = mysqli_query($con, $query);
        $row = mysqli_fetch_assoc($result);
        
        if ($row['last_invoice_number'] !== null) {
            echo $row['last_invoice_number']+1;
        } else {
  ...
  function addSale() {
    ...
    require "db_connection.php";
    if($con) {
      $query = "INSERT INTO sales (CUSTOMER_ID, INVOICE_ID, PRODUCT_ID, QUANTITY, DISCOUNT, TOTAL)
       VALUES('$customer_id', '$invoice_number','$product_id', '$quantity', '$discount', '$total')";
      $result = mysqli_query($con, $query);
  ...
  function addNewInvoice() {
    ...
    require "db_connection.php";
    if($con) {
      $query = "INSERT INTO invoices (INVOICE_ID, PRODUCT_ID, NET_TOTAL, INVOICE_DATE, CUSTOMER_ID, TOTAL_AMOUNT, TOTAL_DISCOUNT) 
      VALUES('$invoice_number', '$product_id', '$net_total', '$invoice_date', '$customer_id', '$total_amount', '$total_discount')";
      $result = mysqli_query($con, $query);
