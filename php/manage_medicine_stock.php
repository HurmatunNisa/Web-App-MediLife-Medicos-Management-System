<?php
  ...
  if($con) {
    ...
    if(isset($_GET["action"]) && $_GET["action"] == "update") {
      ...
      updateMedicineStock($id, $batch_id, $quantity, $mrp,$minlvl, $maxlvl, $rate);
    }
  ...
  function showMedicinesStock($id) {
    ...
    if($con) {
      $seq_no = 0;
      $query = "SELECT * FROM medicines_stock INNER JOIN medicines ON medicines.NAME = medicines_stock.NAME";
      $result = mysqli_query($con, $query);
...
function showEditOptionsRow($seq_no, $row) {
  ...
  <tr>
    ...
    <td>
      <input type="number" class="form-control" value="<?php echo $row['MIN_STOCK_LVL']; ?>" placeholder="Min stock" id="min" onkeyup="checkValue(this.value, 'min_error');">
      <code class="text-danger small font-weight-bold float-right" id="min_error" style="display: none;"></code>
    </td>
    <td>
      <input type="number" class="form-control" value="<?php echo $row['MAX_STOCK_LVL']; ?>" placeholder="Max stock" id="max" onkeyup="checkValue(this.value, 'max_error');">
    ...
    <td>
      <button href="" class="btn btn-success btn-sm" onclick="updateMedicineStock(<?php echo $row['ID']; ?>);">
...
function updateMedicineStock($id, $batch_id,$quantity, $mrp, $minlvl, $maxlvl, $rate) {
  
  require "db_connection.php";
  $query = "UPDATE medicines_stock SET BATCH_ID = '$batch_id',QUANTITY = $quantity, MRP = $mrp,  MIN_STOCK_LVL = $minlvl,  MAX_STOCK_LVL = $maxlvl, RATE = $rate WHERE ID = $id";
  $result = mysqli_query($con, $query);
  if(!empty($result))
